...
use crate::keys::{Keypair, MasterSecret, ProofOfPossession, Sigkey, Verkey};
use crate::signature::Signature;
use crate::{SignatureGroup, SignatureGroupVec, VerkeyGroup, VerkeyGroupVec};
...
fn generate_key_from_shares<R: RngCore + CryptoRng>(
    num_signers: usize,
    mut master_secret_shares: HashMap<usize, FieldElement>,
    rng: &mut R,
    gen: &VerkeyGroup,
    gens: &[SignatureGroup],
) -> Result<Vec<Signer>, ForwardSecureSignatureError> {
    let mut signers = vec![];
    ...
    for i in 0..num_signers {
        ...
        let sigkey_initial = Sigkey::initial_secret_key(gen, gens, &master_secret, rng)?;
...
pub fn trusted_party_SSS_keygen<R: RngCore + CryptoRng>(
    ...
    gen: &VerkeyGroup,
    gens: &[SignatureGroup],
) -> Result<(FieldElement, Vec<Signer>), ForwardSecureSignatureError> {
    let (secret_x, x_shares) = get_shared_secret(threshold, total);
    Ok((
        secret_x,
        generate_key_from_shares(total, x_shares, rng, gen, gens)?,
    ))
}
...
impl ThresholdScheme {
    ...
    pub fn aggregate_sigs(threshold: usize, sigs: Vec<(usize, Signature)>) -> Signature {
        ...
        Signature {
            sigma_1: sigma_1_bases
                .multi_scalar_mul_const_time(sigma_1_exps.as_ref())
                .unwrap(),
            sigma_2: sigma_2_bases
                .multi_scalar_mul_const_time(sigma_2_exps.as_ref())
                .unwrap(),
        }
    }
...
mod tests {
    ...
    use crate::keys::{InMemorySigKeyDatabase, SigManager};
    use crate::signature::Signature;
    use crate::util::{calculate_l, GeneratorSet};
    ...
    fn check_signing_on_random_msgs(
        threshold: usize,
        signers: &[Signer],
        mut sigkey_dbs: Vec<InMemorySigKeyDatabase>,
        mut sk_managers: Vec<SigManager>,
        T: u128,
        l: u8,
        gens: &GeneratorSet,
    ) {
        ...
        for t in 1..=T {
            ...
            assert!(threshold_sig
                .verify(&msg, t, l, gens, &threshold_vk)
                .unwrap());
            for i in 0..threshold {
                sk_managers[i]
                    .simple_update(&gens, &mut rng, &mut sigkey_dbs[i])
                    .unwrap();
            }
        }
    }
    ...
    fn test_verkey_aggregation_shamir_secret_sharing_keygen() {
        ...
        let (secret_x, signers) =
            trusted_party_SSS_keygen(threshold, total, &mut rng, &generators.0, &generators.1)
                .unwrap();
    ...
    fn test_sign_verify_shamir_secret_sharing_keygen() {
        ...
        let mut sk_managers = Vec::<SigManager>::new();
        let mut sigkey_dbs = Vec::<InMemorySigKeyDatabase>::new();
        for i in 0..total {
            let mut db = InMemorySigKeyDatabase::new();
            let sk_manager =
                SigManager::new(T, l, signers[i].sigkey_initial.clone(), &mut db).unwrap();
            sigkey_dbs.push(db);
            sk_managers.push(sk_manager);
        }
        check_signing_on_random_msgs(
            threshold,
            &signers,
            sigkey_dbs,
            sk_managers,
